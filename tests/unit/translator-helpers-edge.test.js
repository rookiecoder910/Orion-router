...
describe("normalizeClaudePassthrough — haiku adaptive thinking (docs 11 §1)", () => {
  ...
  it("folds mid-conversation system messages into user message to preserve prompt cache", () => {
    ...
    expect(out.messages.some((m) => JSON.stringify(m).includes("be brief"))).toBe(true);
