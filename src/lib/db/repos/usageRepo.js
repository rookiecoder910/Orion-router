...
export async function saveRequestUsage(entry) {
  try {
    ...
    db.transaction(() => {
      // ponytail: dedupe only on explicit entry.id; timestamp-based dedupe drops concurrent writes
      if (entry.id) {
        const existing = db.get(`SELECT id, endpoint FROM usageHistory WHERE id = ?`, [entry.id]);
        if (existing) {
